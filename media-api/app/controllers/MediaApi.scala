...
import com.gu.mediaservice.lib.auth.Authentication.{MachinePrincipal, OnBehalfOfPrincipal, Principal, UserPrincipal}
...
class MediaApi(
    ...
    elasticSearch.getImageById(imageId) map {
      case Some(source) if isVisibleToAccessor(request.user, source) =>
        val maybeResult = for {
          ...
          s3Res = Try(s3Client.getObjectV2(config.imgPublishingBucket, key))
  ...
  def unSoftDeleteImage(id: String) = auth.async { implicit request =>
    implicit val instance: Instance = instanceOf(request)
    implicit val logMarker: LogMarker = MarkerMap(
  ...
  def downloadOriginalImage(id: String) = auth.async { implicit request =>
    ...
    elasticSearch.getImageById(id) flatMap {
      case Some(image) if isVisibleToAccessor(request.user, image) => {
        ...
        val s3Object = s3Client.getObjectV2(config.imageBucket, key)
  ...
  )(implicit logMarker: LogMarker, instance: Instance) = {

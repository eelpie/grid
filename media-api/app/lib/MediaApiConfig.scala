...
class MediaApiConfig(resources: GridConfigResources) extends CommonConfigWithElastic(resources) {
  private val configBucketEndpoint = S3.AmazonAwsS3Endpoint
  ...
  private val usageMailBucketEndpoint = S3.AmazonAwsS3Endpoint
  val usageMailBucket: S3Bucket = S3Bucket(string("s3.usagemail.bucket"), S3.AmazonAwsS3Endpoint, usesPathStyleURLs = false, clientFor(usageMailBucketEndpoint))

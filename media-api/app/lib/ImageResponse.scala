...
class ImageResponse(config: MediaApiConfig, s3Client: S3Client, usageQuota: UsageQuota)
  ...
  def imageLinks(id: String, secureUrl: String, securePngUrl: Option[String], withWritePermission: Boolean, valid: Boolean, orientation: Option[Orientation]): List[Link] = {
    ...
    val optimisedPngLinkMaybe = securePngUrl map { case secureUrl => Link("optimisedPng", makeImgopsUri(new URI(secureUrl), orientation)) }

    val optimisedLink = Link("optimised", makeImgopsUri(new URI(secureUrl), orientation))
  ...
  private def makeImgopsUri(uri: URI, orientation: Option[Orientation]): String = {

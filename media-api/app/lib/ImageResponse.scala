...
class ImageResponse(config: MediaApiConfig, s3Client: S3, usageQuota: UsageQuota)
  ...
  def create(
  ...
  )(implicit logMarker: LogMarker, instance: Instance): (JsValue, List[Link], List[Action]) = {
    ...
    val thumbKey = config.thumbBucket.keyFromURL(fileUri)
    def s3SignedThumbUrl = s3Client.signUrl(config.thumbBucket, thumbKey, image, imageType = Thumbnail)

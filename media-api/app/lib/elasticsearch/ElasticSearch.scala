...
class ElasticSearch(
  ...
  private def migrationAwareGetter[T](
    ...
    def getFromCurrentIndex(instance: String) = executeAndLog(
  ...
  def getImageUploaderById(id: String, instance: String)(implicit ex: ExecutionContext): Future[Option[String]] = {
    migrationAwareGetter(
      id,
  ...
  private def aggregateSearch(name: String, params: AggregateSearchParams, aggregation: Aggregation, extract: (String, Aggregations) => Seq[BucketResult],
                              instance: String)(implicit ex: ExecutionContext): Future[AggregateSearchResults] = {
    implicit val logMarker = MarkerMap()
  ...
  private def prepareSearch(query: Query, instance: String): SearchRequest = {

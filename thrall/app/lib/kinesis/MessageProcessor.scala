...
class MessageProcessor(
  ...
  private def updateEmbedding(message: UpdateEmbeddingMessage, logMarker: LogMarker)(implicit ec: ExecutionContext): Future[List[ElasticSearchUpdateResponse]] = {
    implicit val instance: Instance = message.instance
    Future.sequence(es.updateEmbedding(message.id, message.embedding, message.lastModified)(ec, logMarker, instance))
  }
  ...
  private def setupNewInstance(message: CreateInstanceMessage, logMarker: LogMarker)(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    logger.info("Ensuring indexes for create instance message: " + instance.id)
    Future.successful {
      es.ensureIndexExistsAndAliasAssigned(alias = es.imagesCurrentAlias(instance), index = instance.id + "_index")
    }.map { _ =>
      logger.info("Creating Home collection for new instance: " + instance.id)

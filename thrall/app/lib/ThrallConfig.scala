...
class ThrallConfig(resources: GridConfigResources) extends CommonConfigWithElastic(resources) {
  val maybeReaperBucket: Option[S3Bucket] = stringOpt("s3.reaper.bucket").map(S3Bucket(_, S3.AmazonAwsS3Endpoint, usesPathStyleURLs = false, clientFor(S3.AmazonAwsS3Endpoint)))
  ...
  val isInFollowerMode: Boolean = boolean("thrall.followerMode")
}

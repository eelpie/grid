...
class KahunaConfig(resources: GridConfigResources) extends CommonConfig(resources) {
  ...
  val connectSources: Set[String] = getStringSet("security.connectSources") ++ maybeBucketForUIUploads.map { bucketForUIUploads =>
    bucketForUIUploads.bucketURL().toURL.toExternalForm
  } ++ telemetryUri

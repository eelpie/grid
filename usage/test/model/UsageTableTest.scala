  ...
  describe("UsageTable") {
    ...
    it("should be able to query by usage id") {
      ...
      val usage = MediaUsage(
        ...
        Some(ChildUsageMetadata(
          addedBy = "Some One",
          childMediaId = "some-other-media")),
        DateTime.now()
      )
    ...
    it("should be able to match a usage group") {
      ...
      val eventualResult = store.matchUsageGroup(WithLogMarker((usageGroup, instance))(MarkerMap())).toList.toBlocking.toFuture

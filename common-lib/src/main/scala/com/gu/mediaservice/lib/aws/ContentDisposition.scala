...
trait ContentDisposition extends GridLogging {
  ...
  private def getContentDisposition(filename: String, fallback: String): String = {
    ...
    val filenameISO = fallback
    val filenameUTF8 = encodedFilenameUTF8(filename)
  ...
  private def encodedFilenameUTF8(filename: String) = {

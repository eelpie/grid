...
class ImageOperations(playPath: String) extends GridLogging {
  ...
  def createCrops(sourceImage: VImage, dimensionList: List[Dimensions], imageId: String, bounds: Bounds, cropType: MimeType, tempDir: File, cropQuality: Int
                 )(implicit logMarker: LogMarker, instance: Instance, arena: Arena): Future[Seq[(File, String, Dimensions)]] = {
  ...
  def createThumbnailVips(browserViewableImage: BrowserViewableImage,
    ...
    Future {
      ...
      try {
        ...
        val inMemoryCopy = toInMemoryCopy(thumbnail)
  ...
  private def toInMemoryCopy(thumbnail: VImage) = {
    thumbnail.copyMemory()
  }

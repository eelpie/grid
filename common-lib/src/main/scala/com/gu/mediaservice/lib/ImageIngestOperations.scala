...
class ImageIngestOperations(imageBucket: String, thumbnailBucket: String, config: CommonConfig, isVersionedS3: Boolean = false)
  ...
  def deleteOriginals(ids: Set[String], instance: Instance) = bulkDeleteV2(imageBucket, ids.map(id => fileKeyFromId(id, instance)).toList)
  ...
  def deleteThumbnails(ids: Set[String], instance: Instance) = bulkDeleteV2(thumbnailBucket, ids.map(id => fileKeyFromId(id, instance)).toList)
  ...
  def deletePNGs(ids: Set[String], instance: Instance) = bulkDeleteV2(imageBucket, ids.map(id => optimisedPngKeyFromId(id, instance)).toList)

  def doesOriginalExist(id: String, instance: Instance): Boolean =
    client.doesObjectExist(imageBucket, fileKeyFromId(id, instance))

  def doesOriginalExistV2(id: String, instance: Instance): Boolean = {
    this.doesObjectExistV2(imageBucket, fileKeyFromId(id, instance))
  }

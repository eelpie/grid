...
object GridClient extends LazyLogging {
  def apply(services: Services, originUri: Instance => String)(implicit wsClient: WSClient): GridClient =
...
class GridClient(services: Services, originDomain: Instance => String)(implicit wsClient: WSClient) extends LazyLogging {
  ...
  def postUsage(usageType: String, data: JsObject, authFn: WSRequest => WSRequest)(implicit ec: ExecutionContext, instance: Instance) = {
    val url = new URL(s"${services.usageBaseUri(instance)}/usages/$usageType")
  ...
  def putArchived(mediaId: String, authFn: WSRequest => WSRequest)(implicit ec: ExecutionContext, instance: Instance) = {
    val url = new URL(s"${services.metadataBaseUri(instance)}/metadata/$mediaId/archived")

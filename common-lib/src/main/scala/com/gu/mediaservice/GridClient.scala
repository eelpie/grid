...
class GridClient(services: Services)(implicit wsClient: WSClient) extends LazyLogging {
  ...
  def creatCollection(name: String, authFn: WSRequest => WSRequest)(implicit ec: ExecutionContext, instance: Instance): Future[Option[Collection]] = {
    ...
    authorisedRequest.post(data).map { response =>
      ...
      validateResponse(response, url) match {
        case Found(json, _) => Some((json \ "data").as[Collection])
        case NotFound(_, _) => None
        case e@Error(_, _, _) => e.logErrorAndThrowException()
      }
    }

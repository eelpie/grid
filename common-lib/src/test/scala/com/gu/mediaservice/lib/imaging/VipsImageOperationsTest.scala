...
class VipsImageOperationsTest extends AnyFunSpec with Matchers with ScalaFutures {
  ...
  describe("thumbnail") {
    it("should write thumbnail to output file") {
      ...
      val eventualThumbnail = new VipsImageOperations("").createThumbnail(browserViewableImageImage, 240, 95, outputFile, None)
    ...
    it("should correct for exif orientation if provided") {
      ...
      val eventualThumbnail = new VipsImageOperations("").createThumbnail(browserViewableImageImage, 2800, 95, outputFile, Some(OrientationMetadata(exifOrientation = Some(6))))
      whenReady(eventualThumbnail) { r =>
        r._1.isFile should be(true)
      }
    }
  ...
  describe("embeddings") {
    it("should produce embedding sources from original images") {
      ...
      whenReady(eventualEmbeddingSource) { maybeSource =>
        arena.close()
        maybeSource.get.length > 100 should be(true)

        val outputFile = new File("/Users/tony/Desktop/embedding-source.jpg")
        FileUtils.writeByteArrayToFile(outputFile, maybeSource.get)
      }
    }
  }
  ...
  describe("resize") {
    it("should output resized image to file in chosen format") {
      ...
      val eventuallyResized = imageOperations.resizeImage(fullSizedImage, Dimensions(1000, 800), 95, outputFile, Jpeg)
    ...
    it("render LAB colour spaces correctly in sRGB") {
      ...
      val eventuallyResized = imageOperations.resizeImage(fullSizedImage, Dimensions(800, 600), 95, outputFile, Jpeg)
    ...
    it("render LAB colour spaces correctly as PNG") {
      ...
      val eventuallyResized = imageOperations.resizeImage(fullSizedImage, Dimensions(800, 600), 95, outputFile, Png)
  ...
  describe("identifyColourModel") {
    it("should return RGB for a JPG image with RGB image data and no embedded profile") {
      ...
      val colourModelFuture = VipsImageOperations.getImageInformation(image)
  ...
  describe("dimensions") {
    it("should return dimensions of horizontal image") {
      ...
      val dimsFuture = VipsImageOperations.getImageInformation(inputFile)
  ...
  describe("orientation") {
    it("should capture exif orientation tag from JPG images") {
      ...
      val orientationFuture = VipsImageOperations.getImageInformation(image)
  ...
  describe("cropping") {
    ...
    it("should create unscaled master crop to resize from full sized images") {
      ...
      val masterCrop = operations.cropImage(fullsizedImage, Bounds(100, 100, 2000, 2400), metadata, None)
    ...
    it("should create files foreach crop size") {
      ...
      val masterCrop = operations.cropImage(fullsizedImage, Bounds(100, 100, 3000, 2000), metadata, None)

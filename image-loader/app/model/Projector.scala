...
class ImageUploadProjectionOps(config: ImageUploadOpsCfg,
  ...
  private def projectOriginalFileAsS3Model(storableOriginalImage: StorableOriginalImage) =
    Future.successful(storableOriginalImage.toProjectedS3Object(config.originalFileBucket, config.originalFileBucketEndpoint))

  private def projectThumbnailFileAsS3Model(storableThumbImage: StorableThumbImage) =
    Future.successful(storableThumbImage.toProjectedS3Object(config.thumbBucket, config.thumbBucketEndpoint))

  private def projectOptimisedPNGFileAsS3Model(storableOptimisedImage: StorableOptimisedImage) =
    Future.successful(storableOptimisedImage.toProjectedS3Object(config.originalFileBucket, config.originalFileBucketEndpoint))

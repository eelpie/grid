...
object Projector {
  ...
  def apply(config: ImageLoaderConfig, imageOps: ImageOperations, s3: S3, processor: ImageProcessor, auth: Authentication, maybeEmbedder: Option[Embedder], optimiseOps: OptimiseOps)(implicit ec: ExecutionContext): Projector
...
class ImageUploadProjectionOps(config: ImageUploadOpsCfg,
  ...
  def projectImageFromUploadRequest(uploadRequest: UploadRequest)
    ...
    val dependenciesWithProjectionsOnly: ImageUploadOpsDependencies = ImageUploadOpsDependencies(
      ...
      tryFetchEmbeddingResult = fetchEmbedding,
  ...
  private def fetchEmbedding(imageId: String, instance: Instance)(implicit ec: ExecutionContext, logMarker: LogMarker): Future[Option[Embedding]] = {
    val key = embeddingKeyFromId(imageId)(instance)
    val doesResultExist = Future { s3.doesObjectExistV2(config.embedSourceBucket, key) } recover { case _ => false }
    doesResultExist.flatMap {
      ...
      case true =>
        Future {
          s3.getObjectAsStringV2(config.embedSourceBucket, key).flatMap { json =>

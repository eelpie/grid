...
object Uploader extends GridLogging {
  ...
  private[model] def uploadAndStoreImage(storeOrProjectOriginalFile: StorableOriginalImage => Future[S3Object],
    ...
    val eventualImage = for {
      ...
      imageInformation <- VipsImageOperations.getImageInformation(uploadRequest.tempFile)
      ...
      thumbDimensions <- {
        maybeThumbDimensions.map { dimensions =>
          ...
          VipsImageOperations.getImageInformation(thumbViewableImage.file).map(_._1)
        }
      }
    ...
    for {
      ...
      _ = imageUpload.embeddingSource.foreach { embeddingSource =>
        ...
        queueImageToEmbed(EmbedderMessage(
          uploadRequest.imageId,
          config.embeddingSourcesBucket.name,
          config.embeddingSourcesBucket.keyFromURL(embeddingSource.uri),
          instance.id,

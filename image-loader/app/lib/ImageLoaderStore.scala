...
class ImageLoaderStore(config: ImageLoaderConfig) extends lib.ImageIngestOperations(config.imageBucket, config.thumbnailBucket, config.embeddingSourcesBucket, config.embeddingsBucket, config) with GridLogging {

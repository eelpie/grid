    ...
    Future.sequence(
      ...
      mediaIdToFilenameMap.map{case (mediaId, filename) =>
        ...
        uploadStatusTable.setStatus(UploadStatusRecord(
          ...
          usageEvents.prepare(instance = instance, image = mediaId, user = uploadedBy)
  ...
  def loadImage(uploadedBy: Option[String], identifiers: Option[String], uploadTime: Option[String], filename: Option[String]): Action[DigestedFile] = {
    ...
    AuthenticatedAndAuthorised.async(bodyParser) { req: Authentication.Request[DigestedFile] =>
      ...
      result map { r =>
        ...
        usageEvents.successfulUpload(instance = instance, image = req.body.digest, filesize = req.body.file.length())
        result
      } recover {
        case NonFatal(e) =>

...
class ProjectorTest extends AnyFreeSpec with Matchers with ScalaFutures with MockitoSugar {
  ...
  private val config = ImageUploadOpsCfg(new File("/tmp"), 256, 85d, S3Bucket("img-bucket", S3.AmazonAwsS3Endpoint, usesPathStyleURLs = false), S3Bucket("thumb-bucket", S3.AmazonAwsS3Endpoint, usesPathStyleURLs = false))

  private val s3 = mock[S3]
  private val auth = mock[Authentication]
  private val projector = new Projector(config, s3, imageOperations, ImageProcessor.identity, auth, new OptimiseWithPngQuant(imageOperations))

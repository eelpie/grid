...
class Crops(config: CropperConfig, store: CropStore, imageOperations: ImageOperations)(implicit ec: ExecutionContext) extends GridLogging {
  ...
  def createCrops(sourceFile: File, dimensionList: List[Dimensions], apiImage: SourceImage, crop: Crop, cropType: MimeType,
                  instance: Instance)(implicit logMarker: LogMarker): Future[List[Asset]] = {
  ...
  def export(apiImage: SourceImage, crop: Crop, instance: Instance)(implicit logMarker: LogMarker): Future[ExportResult] = {
    val source    = crop.specification
    ...
    Stopwatch(s"making crop assets for ${apiImage.id} ${Crop.getCropId(source.bounds)}") {
      for {
        ...
        masterCrop <- createMasterCrop(apiImage, sourceFile, crop, cropType, colourModel, instance)
